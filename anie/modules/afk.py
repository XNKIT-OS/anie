...
from anie.core.anie_database.anie_db_afk import me_afk, get_afk, me_online
...
@anie_on_cmd(command="afk", modlue=mod_file)
async def me_goin_oflin(_, message: Message):
    afk_msg = await e_or_r(anie_message=message, msg_text="`Wait..`")
...
async def back_online_bois(_, message: Message):
    ...
    await me_online()
    await e_or_r(anie_message=message, msg_text=f"**I'm No Longer AFK** \n\n**Afk Time:** `{afk_time}` \n**Reason:** `{a_reason}`")
