...
from anie.core.main_cmd import anie_on_cmd, e_or_r, anie_on_cf
from anie.helpers.pyrogram_help import get_arg, convert_to_image
...
# Telegraph client
telegraph = Telegraph()
telegraph.create_account(short_name="AnKiT")

# Paste text to telegraph
async def paste_text_to_tgraph(title, text):
  try:
    anie_usr = await ANIE.get_me()
    f_name = anie_usr.first_name
    u_name = anie_usr.username
    if title is None:
      title = f_name if f_name is not None else "By AnKiT"
    t_response = telegraph.create_page(title=title, html_content=text, author_name=f_name if f_name is not None else "AnKiT", author_url=f"https://t.me/{u_name}" if u_name is not None else "https://github.com/xNKIT/AniE")
...
@anie_on_cmd(command="telegraph", modlue=mod_file)
async def telegraph_up(_, message: Message):
    tgraph_msg = await e_or_r(anie_message=message, msg_text="`Processing...`")
    ...
    if r_msg:
      ...
      # Stickers
        ...
        r_content = await convert_to_image(message=r_msg, client=Anie)
      # Text messages
      elif r_msg.text:
