...
**Example:**
  ➥ `duck_s`,
   ⤷ Send command with query = `{Config.CMD_PREFIX}duck_s Anie`
  ➥ `google`,
   ⤷ Send command with query = `{Config.CMD_PREFIX}google Anie`
...
@anie_on_cmd(command="duck_s", modlue=mod_file)
async def duckduckg_s(client, message):
    pablo = await e_or_r(anie_message=message, msg_text="`Searcing in DuckDuckGo...`")
...
@anie_on_cmd(command="google", modlue=mod_file)
async def google_s(client, message):
    pablo = await e_or_r(anie_message=message, msg_text="`Searching in Google...`")
    query = get_arg(message)
