...
# Purges
@anie_on_cmd(command="purge", modlue=mod_file, admins_only=True)
async def purge_this(_, message: Message):
  p_msg = await e_or_r(anie_message=message, msg_text="`Processing...`")
  ...
  for mid in range(message.reply_to_message.message_id, message.message_id):
    ...
    if len(mid_list) == 100:
      await ANIE.delete_messages(chat_id=message.chat.id, message_ids=mid_list, revoke=True) # Docs says revoke Defaults to True but...
      mid_list = []
    # Let's check if there are any other messages left to delete. Just like that 0.1% bacteria that can't be destroyed by soap
    if len(mid_list) > 0:
      await ANIE.delete_messages(chat_id=message.chat.id, message_ids=mid_list, revoke=True)


# Bans / Kicks
@anie_on_cmd(command="ban", modlue=mod_file, admins_only=True)
async def ban_usr(_, message: Message):
  b_k_msg = await e_or_r(anie_message=message, msg_text="`Processing...`")
  r_msg = message.reply_to_message
  just_kick = False
  is_me = await ANIE.get_me()
...
# Unbans
@anie_on_cmd(command="unban", modlue=mod_file, admins_only=True)
async def unban_usr(_, message: Message):
  u_msg = await e_or_r(anie_message=message, msg_text="`Processing...`")
  r_msg = message.reply_to_message
...
# Pin message
@anie_on_cmd(command="pin", modlue=mod_file, admins_only=True, only_groups=True)
async def pin_msg(_, message: Message):
  pin_msg = await e_or_r(anie_message=message, msg_text="`Processing...`")
  r_msg = message.reply_to_message
  args = get_arg(message)
  if not r_msg:
...
# Unpin message
@anie_on_cmd(command="unpin", modlue=mod_file, admins_only=True, only_groups=True)
async def unpin_msg(_, message: Message):
  unpin_msg = await e_or_r(anie_message=message, msg_text="`Processing...`")
  r_msg = message.reply_to_message
  args = get_arg(message)
  if args and (args == "-all"):
    chat_id = message.chat.id
    await ANIE.unpin_all_chat_messages(chat_id)
