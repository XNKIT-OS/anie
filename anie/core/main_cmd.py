...
from anie.core.anie_database.anie_db_sudos import get_sudos
...
def anie_on_cmd(
    ...
    if no_sudos:
        anie_filter = (filters.me & filters.command(command, Config.CMD_PREFIX) & ~filters.via_bot & ~filters.forwarded)
    else:
        anie_filter = (filters.user(SUDO_IDS) & filters.command(command, Config.CMD_PREFIX) & ~filters.via_bot & ~filters.forwarded)
    def decorate_anie(func):
        async def x_wrapper(client, message):
            anie_chat_type = message.chat.type
            if admins_only:
                if anie_chat_type in ["group", "supergroup", "channel"]:
                    usr = await ANIE.get_me()
...
**Traceback:**
`{e}`
Forward this to @AbOuT_xNKIT
"""
                if len(error_text) > 4000:
                    file = open("error_anie.txt", "w+")
                    file.write(error_text)
                    file.close()
                    await ANIE.send_document(LOG_CHANNEL_ID, "error_anie.txt", caption="`Error of Anie`")
                    os.remove("error_anie.txt")
                else:
                    await ANIE.send_message(chat_id=LOG_CHANNEL_ID, text=error_text)
        add_handler(x_wrapper, anie_filter)
        return x_wrapper
    return decorate_anie


# Custom filter handling (Credits: Friday Userbot)
def anie_on_cf(custom_filters):
    def decorate_anie_cf(func):
...
"""
        ...
        ANIE.add_handler(MessageHandler(x_wrapper_cf, filters=custom_filters), group=0)
        return x_wrapper_cf
    return decorate_anie_cf
